...
if ($action === 'register' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $name=$trim=$input['name']??''; $name=trim($name);
    ...
    if (password_score($password)<50) { echo json_encode(['ok'=>false,'message'=>'Password troppo debole (minimo 50/100)']); exit; }
    $st=$pdo->prepare('SELECT id FROM users WHERE email=?'); $st->execute([$email]);
    if ($st->fetch()) { echo json_encode(['ok'=>false,'message'=>'Email gia registrata']); exit; }
    $st=$pdo->prepare('INSERT INTO users (name,email,password_hash,role) VALUES (?,?,?,?)');
    $st->execute([$name,$email,password_hash($password,PASSWORD_DEFAULT),'user']);
...
if ($action === 'login' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $st=$pdo->prepare('SELECT * FROM users WHERE email=?'); $st->execute([$email]);
    $u=$st->fetch();
    if (!$u||!password_verify($password,$u['password_hash'])) { echo json_encode(['ok'=>false,'message'=>'Credenziali non valide']); exit; }
...
// LIST REPORTS (pubblico: ultime 20; admin: tutte)
if ($action === 'reports') {
    if (is_admin()) {
...
// ADMIN: aggiorna status segnalazione
if ($action === 'update_report_status' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$id||!in_array($status,$allowed,true)) { echo json_encode(['ok'=>false,'message'=>'Dati non validi']); exit; }
    $st=$pdo->prepare('UPDATE reports SET status=? WHERE id=?'); $st->execute([$status,$id]);
...
if ($action === 'delete_report' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    require_login_json(); require_admin_json();
    $id=(int)($input['id']??0);
    $st=$pdo->prepare('DELETE FROM reports WHERE id=?'); $st->execute([$id]);
    echo json_encode(['ok'=>true]); exit;
}
...
if ($action === 'save_password' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    require_login_json();
    $site=trim($input['site_name']??''); $user=trim($input['username']??''); $pw=trim($input['password_plain']??'');
    if (!$site||!$user||!$pw) { echo json_encode(['ok'=>false,'message'=>'Campi obbligatori mancanti']); exit; }
