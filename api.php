...
function password_score(string $password): int {
    ...
    if (preg_match('/[^\w\s]/', $password))    $score += 15;
...
if ($action === 'register' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$name || !$email || !$password) { echo json_encode(['ok'=>false,'message'=>'Tutti i campi sono obbligatori']); exit; }
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) { echo json_encode(['ok'=>false,'message'=>'Email non valida']); exit; }
    if (password_score($password) < 50) { echo json_encode(['ok'=>false,'message'=>'Password troppo debole (min 50/100)']); exit; }
    $stmt = $pdo->prepare('SELECT id FROM users WHERE email = ?');
    $stmt->execute([$email]);
    if ($stmt->fetch()) { echo json_encode(['ok'=>false,'message'=>'Email gia registrata']); exit; }
...
if ($action === 'login' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt->execute([$email]);
    $user = $stmt->fetch();
    if (!$user || !password_verify($password, $user['password_hash'])) { echo json_encode(['ok'=>false,'message'=>'Credenziali non valide']); exit; }
    $_SESSION['user'] = ['id'=>$user['id'],'name'=>$user['name'],'email'=>$user['email']];
    echo json_encode(['ok'=>true,'user'=>$_SESSION['user']]); exit;
}

if ($action === 'logout') { session_destroy(); echo json_encode(['ok'=>true]); exit; }
if ($action === 'session') { echo json_encode(['ok'=>true,'user'=>current_user()]); exit; }

if ($action === 'security_score' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    echo json_encode(['ok'=>true,'score'=>password_score($input['password']??'')]); exit;
}
...
if ($action === 'add_report' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $name        = trim($input['name']        ?? 'Anonimo');
    $category    = trim($input['category']    ?? 'Altro');
    $title       = trim($input['title']       ?? '');
    $description = trim($input['description'] ?? '');
    $priority    = trim($input['priority']    ?? 'Media');
    if (!$title || !$description) { echo json_encode(['ok'=>false,'message'=>'Titolo e descrizione obbligatori']); exit; }
    if (!in_array($priority,['Bassa','Media','Alta'])) $priority='Media';
    $tracking = 'SS-' . strtoupper(substr(bin2hex(random_bytes(4)),0,6));
    $stmt = $pdo->prepare('INSERT INTO reports (user_id,name,category,title,description,priority,status,tracking_code) VALUES (?,?,?,?,?,?,?,?)');
...
if ($action === 'save_password' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$site_name||!$username||!$password_plain) { echo json_encode(['ok'=>false,'message'=>'Campi obbligatori mancanti']); exit; }
    $stmt = $pdo->prepare('INSERT INTO vault_items (user_id,site_name,username,password_plain,notes) VALUES (?,?,?,?,?)');
...
if ($action === 'vault') {
    if (!is_logged_in()) { echo json_encode(['ok'=>true,'items']=>[]]); exit; }
    $stmt = $pdo->prepare('SELECT id,site_name,username,password_plain,notes,created_at FROM vault_items WHERE user_id=? ORDER BY id DESC');
